...
void CAI_PlayerAlly::DisplayDeathMessage( void )
{
	...
	CBaseEntity *pPlayer = UTIL_GetNearestPlayer((CBaseEntity*)this, true);
...
void CAI_PlayerAlly::GatherConditions( void )
{
	...
	CBasePlayer *pLocalPlayer = UTIL_GetNearestPlayer(this, true);
	if (!pLocalPlayer) pLocalPlayer = UTIL_GetNearestPlayer(this);
	...
	case TASK_MOVE_AWAY_PATH:
		{
			...
			if ( HasCondition( COND_PLAYER_PUSHING ) && pPlayer )
			{
...
void CAI_PlayerAlly::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( IsInPlayerSquad() )
	{
		CBasePlayer *player = UTIL_GetNearestPlayer((CBaseEntity*)this, true);
		if ( player )
		{
