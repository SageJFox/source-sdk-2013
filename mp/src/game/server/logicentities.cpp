...
//-----------------------------------------------------------------------------
class CFindCounter : public CLogicalEntity
{
	DECLARE_CLASS(CFindCounter, CLogicalEntity);
private:
...
// Keys
//DEFINE_KEYFIELD(m_nMin, FIELD_FLOAT, "min"),
//DEFINE_KEYFIELD(m_nMax, FIELD_FLOAT, "max"),
...
bool CFindCounter::KeyValue(const char *szKeyName, const char *szValue)
{
	//
	// Setup initial finds.
	//
...
// Output : Current text offset from the top
//-----------------------------------------------------------------------------
int CFindCounter::DrawDebugTextOverlays(void)
{
	int text_offset = BaseClass::DrawDebugTextOverlays();
...
//-----------------------------------------------------------------------------
void CFindCounter::InputAdd(inputdata_t &inputdata)
{
	if (m_bDisabled)
	{
		DevMsg("Find Counter %s ignoring ADD because it is disabled\n", GetDebugName());
		return;
	}
...
//-----------------------------------------------------------------------------
void CFindCounter::InputSubtract(inputdata_t &inputdata)
{
	if (m_bDisabled)
	{
		DevMsg("Math Counter %s ignoring SUBTRACT because it is disabled\n", GetDebugName());
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFindCounter::InputGetValue(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFindCounter::InputEnable(inputdata_t &inputdata)
{
	m_bDisabled = false;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFindCounter::InputDisable(inputdata_t &inputdata)
{
	m_bDisabled = true;
}
...
// Input  : fNewValue - Value to set.
//-----------------------------------------------------------------------------
void CFindCounter::UpdateOutValue(CBaseEntity *pActivator, int find, int nNewValue)
{
	if ((m_nMin != 0) || (m_nMax != 0))
	{
		//
...
bool CFindCounter::InputAddRandom(CBaseEntity *pActivator)
{
