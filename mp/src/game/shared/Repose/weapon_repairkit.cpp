...
#ifdef CLIENT_DLL
#define CWeaponRepairkit C_WeaponRepairkit
#endif

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

#define REPAIRKIT_RATE_BASE 0.1f
#define REPAIRKIT_RATE_POSITIVE 0.01f
#define REPAIRKIT_RATE_NEGATIVE 0.02f
#define REPAIRKIT_DENY_SOUNDTIMER 1.0f
#define REPAIRKIT_MAXAMMO_MOD 5

#define	CROWBAR_RANGE	75.0f //range copied from crowbar, hence the name

class CWeaponRepairkit : public CHL2MPMachineGun
{
public:
	DECLARE_CLASS( CWeaponRepairkit, CHL2MPMachineGun );
	...
	float	GetFireRate( void )
	{
		...
		if (pPlayer == NULL || pPlayerStats == NULL)
			return REPAIRKIT_RATE_BASE * 10.0f; //return a really slow firerate so we can tell that something's not right
		//define our rate of fire
		float rate = REPAIRKIT_RATE_BASE;
		int mod = pPlayerStats->checkMod(INT);
		mod > 0 ? rate -= (REPAIRKIT_RATE_POSITIVE * mod) : rate -= (REPAIRKIT_RATE_NEGATIVE * mod);
		return rate;
	}	
#ifndef CLIENT_DLL
...
private:
	CWeaponRepairkit( const CWeaponRepairkit & );
...
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponRepairkit, DT_WeaponRepairkit )

BEGIN_NETWORK_TABLE( CWeaponRepairkit, DT_WeaponRepairkit )
#ifdef CLIENT_DLL
...
#ifdef CLIENT_DLL
BEGIN_PREDICTION_DATA(CWeaponRepairkit)
...
LINK_ENTITY_TO_CLASS( weapon_repairkit, CWeaponRepairkit );
PRECACHE_WEAPON_REGISTER(weapon_repairkit);

#ifndef CLIENT_DLL
acttable_t	CWeaponRepairkit::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponRepairkit);
#endif

//=========================================================
CWeaponRepairkit::CWeaponRepairkit( )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRepairkit::Precache( void )
{
#ifndef CLIENT_DLL
...
//-----------------------------------------------------------------------------
void CWeaponRepairkit::Equip( CBaseCombatCharacter *pOwner )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRepairkit::FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator, Vector &vecShootOrigin, Vector &vecShootDir)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRepairkit::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	switch (pEvent->event)
	{
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CWeaponRepairkit::GetPrimaryAttackActivity( void )
{
	/*if ( m_nShotsFired < 2 )
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponRepairkit::AddViewKick( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponRepairkit::PrimaryAttack( void )
{
	...
	if (!pPlayer)
		return;
#ifndef CLIENT_DLL
	if (!pPlayer->IsSynthetic()) //can only heal synthetics
		return;
#endif // !CLIENT_DLL
	...
	if (pPlayer->HealthFraction() >= 1.0f)
		return PlayDenySound(REPAIRKIT_DENY_SOUNDTIMER);
...
//-----------------------------------------------------------------------------
void CWeaponRepairkit::SecondaryAttack(void)
{
	// Only the player fires this way so we can cast
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
	if (!pPlayer)
	...
	if (pTarget == NULL) //we're aiming at something we can't heal
		return PlayDenySound(REPAIRKIT_DENY_SOUNDTIMER);
#ifndef CLIENT_DLL
	if (!pTarget->IsSynthetic()) //our target isn't synthetic, we can't heal them
		return PlayDenySound(REPAIRKIT_DENY_SOUNDTIMER);
#endif // !CLIENT_DLL
...
//-----------------------------------------------------------------------------
const WeaponProficiencyInfo_t *CWeaponRepairkit::GetProficiencyValues()
{
...
//-----------------------------------------------------------------------------
void CWeaponRepairkit::PlayDenySound(float flDelay)
{
#ifndef CLIENT_DLL
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponRepairkit::StartReload(void)
{
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponRepairkit::Reload(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponRepairkit::FinishReload(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponRepairkit::FillClip(void)
{
	CBaseCombatCharacter *pOwner = GetOwner();
...
//-----------------------------------------------------------------------------
void CWeaponRepairkit::ItemPostFrame(void)
{
	...
	if ((m_bDelayedFire2 || pOwner->m_nButtons & IN_ATTACK2) && (m_flNextPrimaryAttack <= gpGlobals->curtime))
	{
		...
		if ((m_iClip1 <= 0 && UsesClipsForAmmo1()))
		{
			if (!pOwner->GetAmmoCount(m_iPrimaryAmmoType))
				PlayDenySound(REPAIRKIT_DENY_SOUNDTIMER);
			else
				StartReload();
		}
		else
